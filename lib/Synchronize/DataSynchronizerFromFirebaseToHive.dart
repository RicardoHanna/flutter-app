...
class DataSynchronizerFromFirebaseToHive {
  ...
  Future<String> synchronizeData(List<String> itemCodes) async {
    try {
      // Fetch data from Firestore
      var firestoreItems = await _firestore
          .collection('Items')
      ...
      // Synchronize data
      String error = await _synchronizeItems(firestoreItems.docs, itemsBox);
  ...
  Future<String> _synchronizeItems(
    ...
    try {
      ...
      return '$e';
      //Hek mni2riya ma7al ma 3milnela call 2iza kenit false ya3ne fi error
      ScaffoldMessenger(
          child: Text("Error synchronizing items from Firebase to Hive: $e"));
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<List<String>> retrievePriceList(List<String> itemCodes) async {
    ...
    try {
      ...
      ScaffoldMessenger(child: Text("Error retrieving price list Codes $e"));
    }
    return priceLists;
  }

  Future<String> synchronizeDataPriceLists(List<String> priceLists) async {
    try {
      // Fetch data from Firestore
      var firestoreItems = await _firestore
          .collection('PriceList')
          .where('plCode', whereIn: priceLists)
          .get();
  ...
  Future<String> _synchronizePriceList(
    ...
    try {
      ...
      // Check for items in Hive that don't exist in Firestore and delete them
      Set<String> firestorePriceCodes =
          Set.from(firestorePriceLists.map((doc) => doc['plCode']));
...
// Identify items in Hive that don't exist in Firestore
      Set<String> itemsToDelete =
          hivePriceCodes.difference(firestorePriceCodes);
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<String> synchronizeDataItemPrice(List<String> itemCodes) async {
    try {
      // Fetch data from Firestore
      var firestoreItems = await _firestore
          .collection('ItemsPrices')
  ...
  Future<String> _synchronizeItemPrice(
...
// If the item exists in Hive, update it if needed
      ...
      Set<String> firestorePriceItemKeys = Set.from(firestoreItemPrice
          .map((doc) => '${doc['plCode']}${doc['itemCode']}'));
      Set<String> hivePriceItemKeys = Set.from(itempriceBox.keys);

      // Identify items in Hive that don't exist in Firestore
      Set<String> itemsToDelete =
          hivePriceItemKeys.difference(firestorePriceItemKeys);
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<String> synchronizeDataItemAttach(List<String> itemCodes) async {
    try {
      // Fetch data from Firestore
      var firestoreItems = await _firestore
          .collection('ItemAttach')
          .where('itemCode', whereIn: itemCodes)
          .get();
  ...
  Future<String> _synchronizeItemAttach(
    ...
    try {
      ...
      for (var doc in firestoreItemAttach) {
        ...
        if (hiveAttachItem == null) {
          var newAttachItem = ItemAttach(doc['itemCode'], doc['attachmentType'],
              doc['attachmentPath'], doc['note'], doc['cmpCode']);
      ...
      // Identify items in Hive that don't exist in Firestore
      Set<String> itemsToDelete =
          hiveItemAttachCodes.difference(firestoreItemAttachCodes);
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<String> synchronizeDataItemGroup(List<String> itemGroupCodes) async {
    try {
      // Fetch data from Firestore
      var firestoreItems = await _firestore
          .collection('ItemGroup')
          .where('groupCode', whereIn: itemGroupCodes)
          .get();
  ...
  Future<String> _synchronizeItemGroup(
    ...
    try {
      ...
      for (var doc in firestoreItemGroup) {
        ...
        if (hiveGroupItem == null) {
          var newGroupItem = ItemGroup(doc['groupCode'], doc['groupName'],
              doc['groupFName'], doc['cmpCode']);
          await itemgroupBox.put('$groupCode$cmpCode', newGroupItem);
        }
        // If the item exists in Hive, update it if needed
        else {
          var updatedGroupItem = ItemGroup(doc['groupCode'], doc['groupName'],
              doc['groupFName'], doc['cmpCode']);
          // Update the item in Hive
...
// Check for items in Hive that don't exist in Firestore and delete them
      Set<String> firestoreItemGroupCodes = Set.from(firestoreItemGroup
          .map((doc) => '${doc['groupCode']}${doc['cmpCode']}'));
      Set<String> hiveItemGroupCodes = Set.from(itemgroupBox.keys);

      // Identify items in Hive that don't exist in Firestore
      Set<String> itemsToDelete =
          hiveItemGroupCodes.difference(firestoreItemGroupCodes);
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<String> synchronizeDataItemCateg(List<String> itemCateg) async {
    try {
      // Fetch data from Firestore
      var firestoreItems = await _firestore
          .collection('ItemCateg')
          .where('categCode', whereIn: itemCateg)
          .get();
  ...
  Future<String> _synchronizeItemCateg(
    ...
    try {
      ...
      for (var doc in firestoreItemCateg) {
        ...
        if (hiveCategItem == null) {
          var newCategItem = ItemCateg(doc['categCode'], doc['categName'],
              doc['categFName'], doc['cmpCode']);
          await itemcategBox.put('$categCode$cmpCode', newCategItem);
        }
        // If the item exists in Hive, update it if needed
        else {
          var updatedCategItem = ItemCateg(doc['categCode'], doc['categName'],
              doc['categFName'], doc['cmpCode']);
          // Update the item in Hive
      ...
      // Check for items in Hive that don't exist in Firestore and delete them
      Set<String> firestoreItemCategCodes = Set.from(firestoreItemCateg
          .map((doc) => '${doc['categCode']}${doc['cmpCode']}'));
      Set<String> hiveItemCategCodes = Set.from(itemcategBox.keys);

      // Identify items in Hive that don't exist in Firestore
      Set<String> itemsToDelete =
          hiveItemCategCodes.difference(firestoreItemCategCodes);
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<String> synchronizeDataItemBrand(List<String> itemBrand) async {
    try {
      // Fetch data from Firestore
      var firestoreItems = await _firestore
          .collection('ItemBrand')
          .where('brandCode', whereIn: itemBrand)
          .get();
  ...
  Future<String> _synchronizeItemBrand(
    ...
    try {
      ...
      for (var doc in firestoreItemBrand) {
        ...
        if (hiveBrandItem == null) {
          var newBrandItem = ItemBrand(doc['brandCode'], doc['brandName'],
              doc['brandFName'], doc['cmpCode']);
          await itembrandBox.put('$brandCode$cmpCode', newBrandItem);
        }
        // If the item exists in Hive, update it if needed
        else {
          var updatedBrandItem = ItemBrand(doc['brandCode'], doc['brandName'],
              doc['brandFName'], doc['cmpCode']);
          // Update the item in Hive
      ...
      // Check for items in Hive that don't exist in Firestore and delete them
      Set<String> firestoreItemBrandCodes = Set.from(firestoreItemBrand
          .map((doc) => '${doc['brandCode']}${doc['cmpCode']}'));
      Set<String> hiveItemBrandCodes = Set.from(itembrandBox.keys);

      // Identify items in Hive that don't exist in Firestore
      Set<String> itemsToDelete =
          hiveItemBrandCodes.difference(firestoreItemBrandCodes);
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<String> synchronizeDataItemUOM(List<String> itemCodes) async {
    try {
      // Fetch data from Firestore
      var firestoreItems = await _firestore
          .collection('ItemUOM')
      ...
      // Synchronize data
      String err = await _synchronizeItemUOM(firestoreItems.docs, itemuomBox);
      // Synchronize other data if needed
      if (err != '') {
        return err;
      }
  ...
  Future<String> _synchronizeItemUOM(
    ...
    try {
      ...
      for (var doc in firestoreItemUOM) {
        ...
        // If the item doesn't exist in Hive, add it
        if (hiveuomItem == null) {
          var newUOMItem = ItemUOM(doc['itemCode'], doc['uom'],
              doc['qtyperUOM'].toDouble(), doc['barCode'], doc['cmpCode']);
          await itemuomBox.put('$uom$itemCode$cmpCode', newUOMItem);
        }
        // If the item exists in Hive, update it if needed
        else {
          var updatedUOMItem = ItemUOM(doc['itemCode'], doc['uom'],
              doc['qtyperUOM'].toDouble(), doc['barCode'], doc['cmpCode']);
          // Update the item in Hive
...
// Check for items in Hive that don't exist in Firestore and delete them
      Set<String> firestoreItemUOMCodes = Set.from(firestoreItemUOM
          .map((doc) => '${doc['uom']}${doc['itemCode']}${doc['cmpCode']}'));
      ...
      // Identify items in Hive that don't exist in Firestore
      Set<String> itemsToDelete =
          hiveItemUOMCodes.difference(firestoreItemUOMCodes);
      print(itemsToDelete);
      // Delete items in Hive that don't exist in Firestore
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<String> synchronizeDataUserPL() async {
    try {
      // Fetch data from Firestore
      ...
      // Synchronize data
      String err = await _synchronizeUserPL(firestoreItems.docs, userplBox);
      // Synchronize other data if needed
      if (err != '') {
        return err;
      }
  ...
  Future<String> _synchronizeUserPL(
    ...
    try {
      ...
      // Identify items in Hive that don't exist in Firestore
      Set<String> itemsToDelete =
          hiveUserPLCodes.difference(firestoreUserPLCodes);
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<String> synchronizeDataUser() async {
    try {
      // Fetch data from Firestore
      ...
      // Synchronize data
      String err = await _synchronizeUsers(firestoreItems.docs);
      // Synchronize other data if needed
      if (err != '') {
        return err;
      }
  ...
  Future<String> _synchronizeUsers(
    List<QueryDocumentSnapshot<Map<dynamic, dynamic>>> firestoreUsers,
  ) async {
    try {
      var userBox = await Hive.openBox('userBox');

      // Get the list of emails from Firestore users
      Set<dynamic> firestoreUserEmails =
          Set.from(firestoreUsers.map((doc) => doc['usercode']));

      // Get the list of emails in the Hive box
      Set<dynamic> hiveUserEmails = Set.from(userBox.keys);

      // Identify emails in Hive that don't exist in Firestore
      Set<dynamic> emailsToDelete =
          hiveUserEmails.difference(firestoreUserEmails);

      // Iterate over Firestore documents
      for (var doc in firestoreUsers) {
        var usercode = doc['usercode'];
        var userData = doc.data() as Map<dynamic, dynamic>;
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<String> synchronizeDataUserGroup() async {
    try {
      // Fetch data from Firestore
  ...
  Future<String> _synchronizeUsersGroup(
    ...
    try {
      ...
      // Check for items in Hive that don't exist in Firestore and delete them
      Set<dynamic> firestoreUserGroupCodes =
      ...
      Set<dynamic> itemsToDelete =
          hiveUserGroupCodes.difference(firestoreUserGroupCodes);
      ...
      print('Error synchronizing Users Group from Firebase to Hive: $e');
      return '$e';
    }
  }
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<String> synchronizeDataUserGroupTranslations() async {
    try {
      // Fetch data from Firestore
      ...
      // Synchronize data
      String err = await _synchronizeUsersTranslations(
          firestoreItems.docs, translationBox);
      // Synchronize other data if needed
      if (err != '') {
        return err;
      }
  ...
  Future<String> _synchronizeUsersTranslations(
    List<QueryDocumentSnapshot<Map<String, dynamic>>>
        firestoreUsersTranslations,
...
// Delete items in Hive that don't exist in Firestore
      Set<dynamic> firestoreUserTransCodes =
          Set.from(firestoreUsersTranslations.map((doc) => doc['groupcode']));
      Set<dynamic> hiveUserTransCodes = Set.from(translationBox.keys);

      // Identify items in Hive that don't exist in Firestore
      Set<dynamic> itemsToDelete =
          hiveUserTransCodes.difference(firestoreUserTransCodes);
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<String> synchronizeDataMenu() async {
    try {
      // Fetch data from Firestore
      ...
      // Synchronize data
      String err = await _synchronizeMenu(
          firestoreItems.docs, menuBox, userGroupBox, syncGroupBox);
  ...
  Future<String> _synchronizeMenu(
    ...
    try {
      ...
      for (var doc in firestoreMenus) {
        ...
        await _synchronizeSubMenu(
            menucode, doc.reference.collection('usergroups'), userGroupBox);
        await synchronizeIESubMenu(menucode,
            doc.reference.collection('syncronizesubmenu'), syncGroupBox);
      }
  ...
  Future<void> _synchronizeSubMenu(
      int menucode,
      CollectionReference usergroupsCollection,
      Box<AdminSubMenu> userGroupBox) async {
    try {
...
// Identify items in Hive that don't exist in Firestore
Set<dynamic> itemsToDelete = hiveUserGroupCodes.difference(firestoreUserGroupCodes);
...
//-----
  ...
  Future<void> synchronizeIESubMenu(
      int menucode,
      CollectionReference syncgroupsCollection,
      Box<SynchronizeSubMenu> syncGroupBox) async {
    try {
      ...
      Set<dynamic> firestoreSyncGroupCodes = Set.from(
          firestoreUserGroups.docs.map((doc) => doc['syncronizecode']));
      Set<dynamic> hiveSyncGroupCodes = Set.from(syncGroupBox.keys);

// Identify items in Hive that don't exist in Firestore
      Set<dynamic> itemsToDelete =
          hiveSyncGroupCodes.difference(firestoreSyncGroupCodes);
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<String> synchronizeDataAuthorization() async {
    try {
      // Fetch data from Firestore
      ...
      // Synchronize data
      String err = await _synchronizeAutho(firestoreItems.docs, authoBox);
      // Synchronize other data if needed
      if (err != '') {
        return err;
      }
  ...
  Future<String> _synchronizeAutho(
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<String> synchronizeDataGeneralSettings() async {
    try {
      // Fetch data from Firestore
  ...
  Future<String> _synchronizeSystem(
    List<QueryDocumentSnapshot<Map<String, dynamic>>> firestoreAutho,
    ...
    try {
      ...
      Set<dynamic> firestoreSystemCodes =
          Set.from(firestoreAutho.map((doc) => doc['groupcode']));
      Set<dynamic> hiveSystemCodes = Set.from(system.keys);

// Identify items in Hive that don't exist in Firestore
      Set<dynamic> itemsToDelete =
          hiveSystemCodes.difference(firestoreSystemCodes);
...
// Delete items in Hive that don't exist in Firestore
      ...
      print(
          'Error synchronizing System General Settings from Firebase to Hive: $e');
      return '$e';
    }
  }
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<String> synchronizeCompanies() async {
    try {
      // Fetch data from Firestore
      var firestoreCompanies =
  ...
  Future<String> _synchronizeCompanies(
    ...
    try {
      ...
      Set<String> firestoreCompanyCodes =
          Set.from(firestoreCompanies.map((doc) => doc['cmpCode']));
...
// Identify companies in Hive that don't exist in Firestore
      Set<String> companiesToDelete =
          hiveCompanyCodes.difference(firestoreCompanyCodes);
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<String> synchronizeDepartements() async {
    try {
      // Fetch data from Firestore
      var firestoreDepartements =
      ...
      // Synchronize data
      String err = await _synchronizeDepartements(
          firestoreDepartements.docs, departementsBox);
      ...
      print(
          'Error synchronizing data from Firebase to Hive for Departements: $e');
      return '$e';
    }
  }

  Future<String> _synchronizeDepartements(
    ...
    try {
      ...
      Set<String> firestoreDepartementKeys = Set.from(firestoreDepartements
          .map((doc) => '${doc['cmpCode']}${doc['depCode']}'));
      Set<String> hiveDepartementKeys = Set.from(departements.keys);

// Identify departments in Hive that don't exist in Firestore
      Set<String> departementsToDelete =
          hiveDepartementKeys.difference(firestoreDepartementKeys);
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<String> synchronizeExchangeRates() async {
    try {
      // Fetch data from Firestore
      var firestoreExchangeRates =
          await FirebaseFirestore.instance.collection('ExchangeRate').get();

      // Open Hive box
      var exchangeRateBox = await Hive.openBox<ExchangeRate>('exchangeRateBox');

      // Synchronize data
      String err = await _synchronizeExchangeRates(
          firestoreExchangeRates.docs, exchangeRateBox);
      ...
      print(
          'Error synchronizing data from Firebase to Hive for ExchangeRate: $e');
      return '$e';
    }
  }

  Future<String> _synchronizeExchangeRates(
    ...
    try {
      ...
      for (var doc in firestoreExchangeRates) {
        var cmpCode = doc[
            'cmpCode']; // Replace 'yourKey' with the actual key for the exchange rate
      ...
      Set<String> firestoreExchangeRateKeys = Set.from(firestoreExchangeRates
          .map((doc) => '${doc['cmpCode']}${doc['curCode']}'));
...
// Identify exchange rates in Hive that don't exist in Firestore
      Set<String> exchangeRatesToDelete =
          hiveExchangeRateKeys.difference(firestoreExchangeRateKeys);
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<String> synchronizeCurrencies() async {
    try {
      // Fetch data from Firestore
      var firestoreCurrencies =
      ...
      // Synchronize data
      String err = await _synchronizeCurrencies(firestoreCurrencies.docs, currenciesBox);
      if(err!=''){
        return err;
      }
      ...
      print(
          'Error synchronizing data from Firebase to Hive for Currencies: $e');
      return '$e';
    }
  }

  Future<String> _synchronizeCurrencies(
    ...
    try {
      ...
      Set<String> firestoreCurrencyKeys = Set.from(firestoreCurrencies
          .map((doc) => '${doc['cmpCode']}${doc['curCode']}'));
...
// Identify currencies in Hive that don't exist in Firestore
      Set<String> currenciesToDelete =
          hiveCurrencyKeys.difference(firestoreCurrencyKeys);
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<String> synchronizeVATGroups() async {
    try {
      // Fetch data from Firestore
      var firestoreVATGroups =
          await FirebaseFirestore.instance.collection('VATGroups').get();

      // Open Hive box
      var vatGroupsBox = await Hive.openBox<VATGroups>('vatGroupsBox');

      // Synchronize data
      String err = await _synchronizeVATGroups(firestoreVATGroups.docs, vatGroupsBox);
      if(err!=''){
        return err;
      }
  ...
  Future<String> _synchronizeVATGroups(
    ...
    try {
      ...
      Set<String> firestoreVATGroupKeys = Set.from(firestoreVATGroups
          .map((doc) => '${doc['cmpCode']}${doc['vatCode']}'));
...
// Identify VAT groups in Hive that don't exist in Firestore
      Set<String> vatGroupsToDelete =
          hiveVATGroupKeys.difference(firestoreVATGroupKeys);
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<String> synchronizeCustGroups() async {
    try {
      // Fetch data from Firestore
      var firestoreCustGroups =
          await FirebaseFirestore.instance.collection('CustGroups').get();

      // Open Hive box
      var custGroupsBox = await Hive.openBox<CustGroups>('custGroupsBox');

      // Synchronize data
      String err = await _synchronizeCustGroups(firestoreCustGroups.docs, custGroupsBox);
      if(err!=''){
        return err;
      }
      ...
      print(
          'Error synchronizing data from Firebase to Hive for CustGroups: $e');
      return '$e';
    }
  }

  Future<String> _synchronizeCustGroups(
    ...
    try {
      ...
      Set<String> firestoreCustGroupKeys = Set.from(firestoreCustGroups
          .map((doc) => '${doc['cmpCode']}${doc['grpCode']}'));
...
// Identify customer groups in Hive that don't exist in Firestore
      Set<String> custGroupsToDelete =
          hiveCustGroupKeys.difference(firestoreCustGroupKeys);
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> synchronizeCustProperties() async {
    try {
      // Fetch data from Firestore
      var firestoreCustProperties =
          await FirebaseFirestore.instance.collection('CustProperties').get();
      ...
      // Synchronize data
      await _synchronizeCustProperties(
          firestoreCustProperties.docs, custPropertiesBox);

      // Close Hive box
    } catch (e) {
      print(
          'Error synchronizing data from Firebase to Hive for CustProperties: $e');
    }
  }
  ...
  Future<void> _synchronizeCustProperties(
    ...
    try {
      ...
      Set<String> firestoreCustPropertyKeys = Set.from(firestoreCustProperties
          .map((doc) => '${doc['cmpCode']}${doc['propCode']}'));
...
// Identify customer properties in Hive that don't exist in Firestore
      Set<String> custPropertiesToDelete =
          hiveCustPropertyKeys.difference(firestoreCustPropertyKeys);
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> _synchronizeRegions(
    ...
    try {
      ...
      Set<String> firestoreRegionKeys = Set.from(
          firestoreRegions.map((doc) => '${doc['cmpCode']}${doc['regCode']}'));
...
// Identify regions in Hive that don't exist in Firestore
      Set<String> regionsToDelete =
          hiveRegionKeys.difference(firestoreRegionKeys);
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> synchronizeWarehouses() async {
    try {
      ...
      print(
          'Error synchronizing data from Firebase to Hive for Warehouses: $e');
    }
  }
  ...
  Future<void> _synchronizeWarehouses(
    ...
    try {
      ...
      Set<String> firestoreWarehouseKeys = Set.from(firestoreWarehouses
          .map((doc) => '${doc['cmpCode']}${doc['whsCode']}'));
...
// Identify warehouses in Hive that don't exist in Firestore
      Set<String> warehousesToDelete =
          hiveWarehouseKeys.difference(firestoreWarehouseKeys);
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> synchronizePaymentTerms() async {
    try {
      // Fetch data from Firestore
      var firestorePaymentTerms =
          await FirebaseFirestore.instance.collection('PaymentTerms').get();

      // Open Hive box
      var paymentTermsBox = await Hive.openBox<PaymentTerms>('paymentTermsBox');

      // Synchronize data
      await _synchronizePaymentTerms(
          firestorePaymentTerms.docs, paymentTermsBox);
    } catch (e) {
      print(
          'Error synchronizing data from Firebase to Hive for PaymentTerms: $e');
    }
  }
  ...
  Future<void> _synchronizePaymentTerms(
    ...
    try {
      ...
      Set<String> firestorePaymentTermKeys = Set.from(firestorePaymentTerms
          .map((doc) => '${doc['cmpCode']}${doc['ptCode']}'));
...
// Identify payment terms in Hive that don't exist in Firestore
      Set<String> paymentTermsToDelete =
          hivePaymentTermKeys.difference(firestorePaymentTermKeys);
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> synchronizeSalesEmployees(List<String> seCodes) async {
    try {
      // Fetch data from Firestore
      var firestoreSalesEmployees = await FirebaseFirestore.instance
          .collection('SalesEmployees')
          .where('seCode', whereIn: seCodes)
          .get();
      ...
      // Synchronize data
      await _synchronizeSalesEmployees(
          firestoreSalesEmployees.docs, salesEmployeesBox);
    } catch (e) {
      print(
          'Error synchronizing data from Firebase to Hive for SalesEmployees: $e');
    }
  }
  ...
  Future<void> _synchronizeSalesEmployees(
    ...
    try {
      ...
      Set<String> firestoreSalesEmployeeKeys = Set.from(firestoreSalesEmployees
          .map((doc) => '${doc['cmpCode']}${doc['seCode']}'));
...
// Identify sales employees in Hive that don't exist in Firestore
      Set<String> salesEmployeesToDelete =
          hiveSalesEmployeeKeys.difference(firestoreSalesEmployeeKeys);
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> synchronizeSalesEmployeesCustomers(List<String> seCodes) async {
    try {
      // Fetch data from Firestore
      var firestoreSalesEmployeesCustomers = await FirebaseFirestore.instance
      ...
      print(
          'Error synchronizing data from Firebase to Hive for SalesEmployeesCustomers: $e');
    }
  }

  Future<void> _synchronizeSalesEmployeesCustomers(
    List<QueryDocumentSnapshot<Map<String, dynamic>>>
        firestoreSalesEmployeesCustomers,
    ...
    try {
      ...
      for (var doc in firestoreSalesEmployeesCustomers) {
        ...
        var hiveSalesEmployeesCustomers =
            salesEmployeesCustomers.get('$cmpCode$seCode$custCode');
        ...
        if (hiveSalesEmployeesCustomers == null) {
          ...
          await salesEmployeesCustomers.put(
      ...
      Set<String> firestoreSalesEmployeesCustomersKeys = Set.from(
          firestoreSalesEmployeesCustomers.map(
              (doc) => '${doc['cmpCode']}${doc['seCode']}${doc['custCode']}'));
      Set<String> hiveSalesEmployeesCustomersKeys =
          Set.from(salesEmployeesCustomers.keys);
...
// Identify sales employee customers relationships in Hive that don't exist in Firestore
      Set<String> salesEmployeesCustomersToDelete =
          hiveSalesEmployeesCustomersKeys
              .difference(firestoreSalesEmployeesCustomersKeys);
...
// Delete sales employee customers relationships in Hive that don't exist in Firestore
      ...
      print(
          'Error synchronizing SalesEmployeesCustomers from Firebase to Hive: $e');
    }
  }
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> synchronizeSalesEmployeesDepartements(
      List<String> seCodes) async {
    try {
      // Fetch data from Firestore
      var firestoreSalesEmployeesDepartements = await FirebaseFirestore.instance
          .collection('SalesEmployeesDepartments')
          .where('itemCode', whereIn: seCodes)
          .get();
      ...
      await _synchronizeSalesEmployeesDepartements(
          firestoreSalesEmployeesDepartements.docs,
          salesEmployeesDepartementsBox);
    } catch (e) {
      print(
          'Error synchronizing data from Firebase to Hive for SalesEmployeesDepartements: $e');
    }
  }

  Future<void> _synchronizeSalesEmployeesDepartements(
    List<QueryDocumentSnapshot<Map<String, dynamic>>>
        firestoreSalesEmployeesDepartements,
    ...
    try {
      ...
      for (var doc in firestoreSalesEmployeesDepartements) {
        ...
        var hiveSalesEmployeesDepartements =
            salesEmployeesDepartements.get('$cmpCode$seCode$deptCode');
        ...
        if (hiveSalesEmployeesDepartements == null) {
          ...
          await salesEmployeesDepartements.put(
              '$cmpCode$seCode$deptCode', newSalesEmployeesDepartements);
        }
        // If the relationship exists in Hive, update it if needed
        else {
          ...
          // Update the relationship in Hive
          await salesEmployeesDepartements.put(
              '$cmpCode$seCode$deptCode', updatedSalesEmployeesDepartements);
        }
      }
      // Create sets of sales employee departments keys from Firestore and Hive
      Set<String> firestoreSalesEmployeesDepartmentsKeys = Set.from(
          firestoreSalesEmployeesDepartements.map(
              (doc) => '${doc['cmpCode']}${doc['seCode']}${doc['deptCode']}'));
      Set<String> hiveSalesEmployeesDepartmentsKeys =
          Set.from(salesEmployeesDepartements.keys);
...
// Identify sales employee departments relationships in Hive that don't exist in Firestore
      Set<String> salesEmployeesDepartmentsToDelete =
          hiveSalesEmployeesDepartmentsKeys
              .difference(firestoreSalesEmployeesDepartmentsKeys);
...
// Delete sales employee departments relationships in Hive that don't exist in Firestore
      ...
      print(
          'Error synchronizing SalesEmployeesDepartements from Firebase to Hive: $e');
    }
  }
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> synchronizeSalesEmployeesItemsBrands(
    ...
    try {
      // Fetch data from Firestore
      var firestoreSalesEmployeesItemsBrands = await FirebaseFirestore.instance
          .collection('SalesEmployeesItemsBrands')
          .where('seCode', whereIn: seCodes)
          .get();
      ...
      await _synchronizeSalesEmployeesItemsBrands(
          firestoreSalesEmployeesItemsBrands.docs,
          salesEmployeesItemsBrandsBox);
    } catch (e) {
      print(
          'Error synchronizing data from Firebase to Hive for SalesEmployeesItemsBrands: $e');
    }
  }

  Future<void> _synchronizeSalesEmployeesItemsBrands(
    List<QueryDocumentSnapshot<Map<String, dynamic>>>
        firestoreSalesEmployeesItemsBrands,
    ...
    try {
      ...
      for (var doc in firestoreSalesEmployeesItemsBrands) {
        ...
        var hiveSalesEmployeesItemsBrands =
            salesEmployeesItemsBrands.get('$cmpCode$seCode$brandCode');
        ...
        if (hiveSalesEmployeesItemsBrands == null) {
          ...
          await salesEmployeesItemsBrands.put(
      ...
      Set<String> firestoreSalesEmployeesItemsBrandsKeys = Set.from(
          firestoreSalesEmployeesItemsBrands.map(
              (doc) => '${doc['cmpCode']}${doc['seCode']}${doc['brandCode']}'));
      Set<String> hiveSalesEmployeesItemsBrandsKeys =
          Set.from(salesEmployeesItemsBrands.keys);
...
// Identify sales employee items brands relationships in Hive that don't exist in Firestore
      Set<String> salesEmployeesItemsBrandsToDelete =
          hiveSalesEmployeesItemsBrandsKeys
              .difference(firestoreSalesEmployeesItemsBrandsKeys);
...
// Delete sales employee items brands relationships in Hive that don't exist in Firestore
      ...
      print(
          'Error synchronizing SalesEmployeesItemsBrands from Firebase to Hive: $e');
    }
  }
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> synchronizeSalesEmployeesItemsCategories(
    ...
    try {
      // Fetch data from Firestore
      var firestoreSalesEmployeesItemsCategories = await FirebaseFirestore
          .instance
          .collection('SalesEmployeesItemsCategories')
          .where('seCode', whereIn: seCodes)
          .get();
      ...
      await _synchronizeSalesEmployeesItemsCategories(
          firestoreSalesEmployeesItemsCategories.docs,
          salesEmployeesItemsCategoriesBox);
    } catch (e) {
      print(
          'Error synchronizing data from Firebase to Hive for SalesEmployeesItemsCategories: $e');
    }
  }

  Future<void> _synchronizeSalesEmployeesItemsCategories(
    List<QueryDocumentSnapshot<Map<String, dynamic>>>
        firestoreSalesEmployeesItemsCategories,
    ...
    try {
      ...
      for (var doc in firestoreSalesEmployeesItemsCategories) {
        ...
        if (hiveSalesEmployeesItemsCategories == null) {
          ...
          await salesEmployeesItemsCategories.put(
              '$cmpCode$seCode$categCode', newSalesEmployeesItemsCategories);
        }
        // If the relationship exists in Hive, update it if needed
        else {
          var updatedSalesEmployeesItemsCategories =
          ...
          // Update the relationship in Hive
          await salesEmployeesItemsCategories.put('$cmpCode$seCode$categCode',
              updatedSalesEmployeesItemsCategories);
        }
      }
      // Create sets of sales employee items categories keys from Firestore and Hive
      Set<String> firestoreSalesEmployeesItemsCategoriesKeys = Set.from(
          firestoreSalesEmployeesItemsCategories.map(
              (doc) => '${doc['cmpCode']}${doc['seCode']}${doc['categCode']}'));
      Set<String> hiveSalesEmployeesItemsCategoriesKeys =
          Set.from(salesEmployeesItemsCategories.keys);
...
// Identify sales employee items categories relationships in Hive that don't exist in Firestore
      Set<String> salesEmployeesItemsCategoriesToDelete =
          hiveSalesEmployeesItemsCategoriesKeys
              .difference(firestoreSalesEmployeesItemsCategoriesKeys);
...
// Delete sales employee items categories relationships in Hive that don't exist in Firestore
      ...
      print(
          'Error synchronizing SalesEmployeesItemsCategories from Firebase to Hive: $e');
    }
  }
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> synchronizeSalesEmployeesItemsGroups(
    ...
    try {
      // Fetch data from Firestore
      var firestoreSalesEmployeesItemsGroups = await FirebaseFirestore.instance
          .collection('SalesEmployeesItemsGroups')
          .where('seCode', whereIn: seCodes)
          .get();
      ...
      await _synchronizeSalesEmployeesItemsGroups(
          firestoreSalesEmployeesItemsGroups.docs,
          salesEmployeesItemsGroupsBox);
    } catch (e) {
      print(
          'Error synchronizing data from Firebase to Hive for SalesEmployeesItemsGroups: $e');
    }
  }

  Future<void> _synchronizeSalesEmployeesItemsGroups(
    List<QueryDocumentSnapshot<Map<String, dynamic>>>
        firestoreSalesEmployeesItemsGroups,
    ...
    try {
      ...
      for (var doc in firestoreSalesEmployeesItemsGroups) {
        ...
        if (hiveSalesEmployeesItemsGroups == null) {
          ...
          await salesEmployeesItemsGroups.put(
      ...
      Set<String> firestoreSalesEmployeesItemsGroupsKeys = Set.from(
          firestoreSalesEmployeesItemsGroups.map(
              (doc) => '${doc['cmpCode']}${doc['seCode']}${doc['groupCode']}'));
      Set<String> hiveSalesEmployeesItemsGroupsKeys =
          Set.from(salesEmployeesItemsGroups.keys);
...
// Identify sales employee items groups relationships in Hive that don't exist in Firestore
      Set<String> salesEmployeesItemsGroupsToDelete =
          hiveSalesEmployeesItemsGroupsKeys
              .difference(firestoreSalesEmployeesItemsGroupsKeys);
...
// Delete sales employee items groups relationships in Hive that don't exist in Firestore
      ...
      print(
          'Error synchronizing SalesEmployeesItemsGroups from Firebase to Hive: $e');
    }
  }
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> synchronizeSalesEmployeesItems(List<String> seCodes) async {
    try {
      // Fetch data from Firestore
      var firestoreSalesEmployeesItems = await FirebaseFirestore.instance
          .collection('SalesEmployeesItems')
          .where('seCode', whereIn: seCodes)
          .get();
      ...
      print(
          'Error synchronizing data from Firebase to Hive for SalesEmployeesItems: $e');
    }
  }

  Future<void> _synchronizeSalesEmployeesItems(
    List<QueryDocumentSnapshot<Map<String, dynamic>>>
        firestoreSalesEmployeesItems,
    ...
    try {
      ...
      for (var doc in firestoreSalesEmployeesItems) {
        ...
        var hiveSalesEmployeesItems =
            salesEmployeesItems.get('$cmpCode$seCode$itemCode');
      ...
      Set<String> firestoreSalesEmployeesItemsKeys = Set.from(
          firestoreSalesEmployeesItems.map(
              (doc) => '${doc['cmpCode']}${doc['seCode']}${doc['itemCode']}'));
      Set<String> hiveSalesEmployeesItemsKeys =
          Set.from(salesEmployeesItems.keys);
...
// Identify sales employee items relationships in Hive that don't exist in Firestore
      Set<String> salesEmployeesItemsToDelete = hiveSalesEmployeesItemsKeys
          .difference(firestoreSalesEmployeesItemsKeys);
...
// Delete sales employee items relationships in Hive that don't exist in Firestore
      ...
      print(
          'Error synchronizing SalesEmployeesItems from Firebase to Hive: $e');
    }
  }
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> synchronizeUserSalesEmployees() async {
    try {
      // Fetch data from Firestore
      var firestoreUserSalesEmployees = await FirebaseFirestore.instance
          .collection('UsersSalesEmployees')
          .get();
      ...
      print(
          'Error synchronizing data from Firebase to Hive for UserSalesEmployees: $e');
    }
  }

  Future<void> _synchronizeUserSalesEmployees(
    List<QueryDocumentSnapshot<Map<String, dynamic>>>
        firestoreUserSalesEmployees,
    ...
    try {
      ...
      for (var doc in firestoreUserSalesEmployees) {
        ...
        var hiveUserSalesEmployees =
            userSalesEmployees.get('$userCode$cmpCode$seCode');
      ...
      Set<String> firestoreUserSalesEmployeesKeys = Set.from(
          firestoreUserSalesEmployees.map(
              (doc) => '${doc['userCode']}${doc['cmpCode']}${doc['seCode']}'));
      Set<String> hiveUserSalesEmployeesKeys =
          Set.from(userSalesEmployees.keys);

      // Identify user sales employees relationships in Hive that don't exist in Firestore
      Set<String> userSalesEmployeesToDelete = hiveUserSalesEmployeesKeys
          .difference(firestoreUserSalesEmployeesKeys);
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> synchronizeCustomers(List<String> custCodes) async {
    try {
      // Fetch data from Firestore
      var firestoreCustomers = await FirebaseFirestore.instance
  ...
  Future<void> _synchronizeCustomers(
    ...
    try {
      ...
      Set<String> firestoreCustomersKeys = Set.from(firestoreCustomers
          .map((doc) => '${doc['cmpCode']}${doc['custCode']}'));
      ...
      Set<String> customersToDelete =
          hiveCustomersKeys.difference(firestoreCustomersKeys);
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> synchronizeCustomerAddresses(List<String> custCodes) async {
    try {
      // Fetch data from Firestore
      var firestoreAddresses = await FirebaseFirestore.instance
      ...
      // Synchronize data
      await _synchronizeCustomerAddresses(
          firestoreAddresses.docs, addressesBox);
    } catch (e) {
      print(
          'Error synchronizing data from Firebase to Hive for CustomerAddresses: $e');
    }
  }
  ...
  Future<void> _synchronizeCustomerAddresses(
    ...
    try {
      ...
      Set<String> firestoreAddressesKeys = Set.from(firestoreAddresses.map(
          (doc) => '${doc['cmpCode']}${doc['addressID']}${doc['custCode']}'));
      ...
      Set<String> addressesToDelete =
          hiveAddressesKeys.difference(firestoreAddressesKeys);
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> synchronizeCustomerContacts(List<String> custCodes) async {
    try {
      // Fetch data from Firestore
      var firestoreContacts = await FirebaseFirestore.instance
      ...
      print(
          'Error synchronizing data from Firebase to Hive for CustomerContacts: $e');
    }
  }
  ...
  Future<void> _synchronizeCustomerContacts(
    ...
    try {
      ...
      Set<String> firestoreContactsKeys = Set.from(firestoreContacts.map(
          (doc) => '${doc['cmpCode']}${doc['contactID']}${doc['custCode']}'));
      Set<String> hiveContactsKeys = Set.from(contacts.keys);

      // Identify contacts in Hive that don't exist in Firestore
      Set<String> contactsToDelete =
          hiveContactsKeys.difference(firestoreContactsKeys);
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> synchronizeCustomerProperties(List<String> custCodes) async {
    try {
      // Fetch data from Firestore
      var firestoreProperties = await FirebaseFirestore.instance
      ...
      // Synchronize data
      await _synchronizeCustomerProperties(
          firestoreProperties.docs, propertiesBox);
    } catch (e) {
      print(
          'Error synchronizing data from Firebase to Hive for CustomerProperties: $e');
    }
  }
  ...
  Future<void> _synchronizeCustomerProperties(
    ...
    try {
      ...
      Set<String> firestorePropertiesKeys = Set.from(firestoreProperties.map(
          (doc) => '${doc['cmpCode']}${doc['propCode']}${doc['custCode']}'));
      ...
      Set<String> propertiesToDelete =
          hivePropertiesKeys.difference(firestorePropertiesKeys);
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> synchronizeCustomerAttachments(List<String> custCodes) async {
    try {
      // Fetch data from Firestore
      var firestoreAttachments = await FirebaseFirestore.instance
          .collection('CustomerAttachments')
          .where('custCode', whereIn: custCodes)
          .get();
      ...
      // Synchronize data
      await _synchronizeCustomerAttachments(
          firestoreAttachments.docs, attachmentsBox);
    } catch (e) {
      print(
          'Error synchronizing data from Firebase to Hive for CustomerAttachments: $e');
    }
  }
  ...
  Future<void> _synchronizeCustomerAttachments(
    ...
    try {
      ...
      Set<String> firestoreAttachmentsKeys = Set.from(firestoreAttachments
          .map((doc) => '${doc['cmpCode']}${doc['custCode']}'));
      ...
      Set<String> attachmentsToDelete =
          hiveAttachmentsKeys.difference(firestoreAttachmentsKeys);
      ...
      print(
          'Error synchronizing CustomerAttachments from Firebase to Hive: $e');
    }
  }
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> synchronizeCustomerItemsSpecialPrice(
    ...
    try {
      ...
      print(
          'Error synchronizing data from Firebase to Hive for CustomerItemsSpecialPrice: $e');
    }
  }
    ...
    try {
      ...
      for (var doc in firestoreSpecialPrice) {
        ...
        // If the special price exists in Hive, update it if needed
          ...
          await specialPrice.put(
              '$cmpCode$itemCode$custCode$uom', updatedSpecialPrice);
        }
      }
      Set<String> firestoreSpecialPriceKeys = Set.from(
          firestoreSpecialPrice.map((doc) =>
              '${doc['cmpCode']}${doc['itemCode']}${doc['custCode']}${doc['uom']}'));
      ...
      Set<String> specialPricesToDelete =
          hiveSpecialPriceKeys.difference(firestoreSpecialPriceKeys);
      ...
      print(
          'Error synchronizing CustomerItemsSpecialPrice from Firebase to Hive: $e');
    }
  }
...
//-------------------------------------------------------------------------------------------------
  Future<void> synchronizeCustomerBrandsSpecialPrice(
    ...
    try {
      ...
      print(
          'Error synchronizing data from Firebase to Hive for CustomerBrandsSpecialPrice: $e');
    }
  }

  Future<void> _synchronizeCustomerBrandsSpecialPrice(
    List<QueryDocumentSnapshot<Map<String, dynamic>>>
        firestoreBrandsSpecialPrice,
    ...
    try {
      ...
      for (var doc in firestoreBrandsSpecialPrice) {
        ...
        // If the brand special price exists in Hive, update it if needed
          ...
          await brandsSpecialPrice.put(
              '$cmpCode$custCode$brandCode', updatedBrandSpecialPrice);
        }
      }
      Set<String> firestoreBrandsSpecialPriceKeys = Set.from(
          firestoreBrandsSpecialPrice.map((doc) =>
              '${doc['cmpCode']}${doc['custCode']}${doc['brandCode']}'));
      Set<String> hiveBrandsSpecialPriceKeys =
          Set.from(brandsSpecialPrice.keys);

      // Identify brand special prices in Hive that don't exist in Firestore
      Set<String> brandsSpecialPricesToDelete = hiveBrandsSpecialPriceKeys
          .difference(firestoreBrandsSpecialPriceKeys);
      ...
      print(
          'Error synchronizing CustomerBrandsSpecialPrice from Firebase to Hive: $e');
    }
  }
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> synchronizeCustomerGroupsSpecialPrice(
    ...
    try {
      ...
      print(
          'Error synchronizing data from Firebase to Hive for CustomerGroupsSpecialPrice: $e');
    }
  }

  Future<void> _synchronizeCustomerGroupsSpecialPrice(
    List<QueryDocumentSnapshot<Map<String, dynamic>>>
        firestoreGroupsSpecialPrice,
    ...
    try {
      ...
      for (var doc in firestoreGroupsSpecialPrice) {
        ...
        // If the group special price exists in Hive, update it if needed
          ...
          await groupsSpecialPrice.put(
              '$cmpCode$custCode$groupCode', updatedGroupSpecialPrice);
        }
      }
      Set<String> firestoreGroupsSpecialPriceKeys = Set.from(
          firestoreGroupsSpecialPrice.map((doc) =>
              '${doc['cmpCode']}${doc['custCode']}${doc['groupCode']}'));
      Set<String> hiveGroupsSpecialPriceKeys =
          Set.from(groupsSpecialPrice.keys);

      // Identify group special prices in Hive that don't exist in Firestore
      Set<String> groupsSpecialPricesToDelete = hiveGroupsSpecialPriceKeys
          .difference(firestoreGroupsSpecialPriceKeys);
      ...
      print(
          'Error synchronizing CustomerGroupsSpecialPrice from Firebase to Hive: $e');
    }
  }
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> synchronizeCustomerCategSpecialPrice(
    ...
    try {
      ...
      print(
          'Error synchronizing data from Firebase to Hive for CustomerCategSpecialPrice: $e');
    }
  }

  Future<void> _synchronizeCustomerCategSpecialPrice(
    List<QueryDocumentSnapshot<Map<String, dynamic>>>
        firestoreCategSpecialPrice,
    ...
    try {
      ...
      for (var doc in firestoreCategSpecialPrice) {
        ...
        // If the categ special price exists in Hive, update it if needed
          ...
          await categSpecialPrice.put(
              '$cmpCode$custCode$categCode', updatedCategSpecialPrice);
        }
      }
      Set<String> firestoreCategSpecialPriceKeys = Set.from(
          firestoreCategSpecialPrice.map((doc) =>
              '${doc['cmpCode']}${doc['custCode']}${doc['categCode']}'));
      ...
      Set<String> categSpecialPricesToDelete =
          hiveCategSpecialPriceKeys.difference(firestoreCategSpecialPriceKeys);
      ...
      print(
          'Error synchronizing CustomerCategSpecialPrice from Firebase to Hive: $e');
    }
  }
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> synchronizeCustomerGroupItemsSpecialPrice(
    ...
    try {
      // Fetch data from Firestore
      var firestoreGroupItemsSpecialPrice = await FirebaseFirestore.instance
          .collection('CustomerGroupItemsSpecialPrice')
          .where('itemCode', whereIn: itemCodes)
          .get();
      ...
      print(
          'Error synchronizing data from Firebase to Hive for CustomerGroupItemsSpecialPrice: $e');
    }
  }

  Future<void> _synchronizeCustomerGroupItemsSpecialPrice(
    List<QueryDocumentSnapshot<Map<String, dynamic>>>
        firestoreGroupItemsSpecialPrice,
    ...
    try {
      ...
      for (var doc in firestoreGroupItemsSpecialPrice) {
        ...
        // If the group item special price exists in Hive, update it if needed
          ...
          await groupItemsSpecialPrice.put(
              '$cmpCode$custGroupCode$itemCode$uom',
              updatedGroupItemSpecialPrice);
        }
      }
      Set<String> firestoreGroupItemsSpecialPriceKeys = Set.from(
          firestoreGroupItemsSpecialPrice.map((doc) =>
              '${doc['cmpCode']}${doc['custGroupCode']}${doc['itemCode']}${doc['uom']}'));
      ...
      Set<String> groupItemsSpecialPricesToDelete =
          hiveGroupItemsSpecialPriceKeys
              .difference(firestoreGroupItemsSpecialPriceKeys);
      ...
      print(
          'Error synchronizing CustomerGroupItemsSpecialPrice from Firebase to Hive: $e');
    }
  }
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> synchronizeCustomerGroupBrandSpecialPrice(
    ...
    try {
      // Fetch data from Firestore
      var firestoreGroupBrandSpecialPrice = await FirebaseFirestore.instance
          .collection('CustomerGroupBrandSpecialPrice')
      ...
      print(
          'Error synchronizing data from Firebase to Hive for CustomerGroupBrandSpecialPrice: $e');
    }
  }

  Future<void> _synchronizeCustomerGroupBrandSpecialPrice(
    List<QueryDocumentSnapshot<Map<String, dynamic>>>
        firestoreGroupBrandSpecialPrice,
    ...
    try {
      ...
      for (var doc in firestoreGroupBrandSpecialPrice) {
        ...
        // If the group brand special price exists in Hive, update it if needed
          ...
          await groupBrandSpecialPrice.put('$cmpCode$custGroupCode$brandCode',
              updatedGroupBrandSpecialPrice);
        }
      }
      Set<String> firestoreGroupBrandSpecialPriceKeys = Set.from(
          firestoreGroupBrandSpecialPrice.map((doc) =>
              '${doc['cmpCode']}${doc['custGroupCode']}${doc['brandCode']}'));
      ...
      Set<String> groupBrandSpecialPricesToDelete =
          hiveGroupBrandSpecialPriceKeys
              .difference(firestoreGroupBrandSpecialPriceKeys);
      ...
      print(
          'Error synchronizing CustomerGroupBrandSpecialPrice from Firebase to Hive: $e');
    }
  }
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> synchronizeCustomerGroupGroupSpecialPrice(
    ...
    try {
      // Fetch data from Firestore
      var firestoreGroupGroupSpecialPrice = await FirebaseFirestore.instance
          .collection('CustomerGroupGroupSpecialPrice')
          .where('groupCode', whereIn: groupCodes)
          .get();
  ...
  Future<void> _synchronizeCustomerGroupGroupSpecialPrice(
    List<QueryDocumentSnapshot<Map<String, dynamic>>>
        firestoreGroupGroupSpecialPrice,
    ...
    try {
      ...
      for (var doc in firestoreGroupGroupSpecialPrice) {
        ...
        // If the group group special price exists in Hive, update it if needed
          ...
          await groupGroupSpecialPrice.put('$cmpCode$custGroupCode$groupCode',
              updatedGroupGroupSpecialPrice);
        }
      }
      Set<String> firestoreGroupGroupSpecialPriceKeys = Set.from(
          firestoreGroupGroupSpecialPrice.map((doc) =>
              '${doc['cmpCode']}${doc['custGroupCode']}${doc['groupCode']}'));
      ...
      Set<String> groupGroupSpecialPricesToDelete =
          hiveGroupGroupSpecialPriceKeys
              .difference(firestoreGroupGroupSpecialPriceKeys);
      ...
      print(
          'Error synchronizing CustomerGroupGroupSpecialPrice from Firebase to Hive: $e');
    }
  }
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> synchronizeCustomerGroupCategSpecialPrice(
    ...
    try {
      // Fetch data from Firestore
      var firestoreGroupCategSpecialPrice = await FirebaseFirestore.instance
          .collection('CustomerGroupCategSpecialPrice')
  ...
  Future<void> _synchronizeCustomerGroupCategSpecialPrice(
    List<QueryDocumentSnapshot<Map<String, dynamic>>>
        firestoreGroupCategSpecialPrice,
    ...
    try {
      ...
      for (var doc in firestoreGroupCategSpecialPrice) {
        ...
        // If the group categ special price exists in Hive, update it if needed
          ...
          await groupCategSpecialPrice.put('$cmpCode$custGroupCode$categCode',
              updatedGroupCategSpecialPrice);
        }
      }
      Set<String> firestoreGroupCategSpecialPriceKeys = Set.from(
          firestoreGroupCategSpecialPrice.map((doc) =>
              '${doc['cmpCode']}${doc['custGroupCode']}${doc['categCode']}'));
      ...
      Set<String> groupCategSpecialPricesToDelete =
          hiveGroupCategSpecialPriceKeys
              .difference(firestoreGroupCategSpecialPriceKeys);
      ...
      print(
          'Error synchronizing CustomerGroupCategSpecialPrice from Firebase to Hive: $e');
    }
  }
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> _synchronizeCustomerPropItemsSpecialPrice(
    List<QueryDocumentSnapshot<Map<String, dynamic>>>
        firestorePropItemsSpecialPrice,
    ...
    try {
      ...
      for (var doc in firestorePropItemsSpecialPrice) {
        ...
        // If the prop items special price exists in Hive, update it if needed
          ...
          await propItemsSpecialPrice.put('$cmpCode$custPropCode$itemCode$uom',
              updatedPropItemsSpecialPrice);
        }
      }
      Set<String> firestorePropItemsSpecialPriceKeys = Set.from(
          firestorePropItemsSpecialPrice.map((doc) =>
              '${doc['cmpCode']}${doc['custPropCode']}${doc['itemCode']}${doc['uom']}'));
      ...
      Set<String> propItemsSpecialPricesToDelete = hivePropItemsSpecialPriceKeys
          .difference(firestorePropItemsSpecialPriceKeys);
      ...
      print(
          'Error synchronizing CustomerPropItemsSpecialPrice from Firebase to Hive: $e');
    }
  }
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> _synchronizeCustomerPropBrandSpecialPrice(
    List<QueryDocumentSnapshot<Map<String, dynamic>>>
        firestorePropBrandSpecialPrice,
    ...
    try {
      ...
      for (var doc in firestorePropBrandSpecialPrice) {
        ...
        // If the prop brand special price exists in Hive, update it if needed
          ...
          await propBrandSpecialPrice.put(
              '$cmpCode$custPropCode$brandCode', updatedPropBrandSpecialPrice);
        }
      }
      Set<String> firestorePropBrandSpecialPriceKeys = Set.from(
          firestorePropBrandSpecialPrice.map((doc) =>
              '${doc['cmpCode']}${doc['custPropCode']}${doc['brandCode']}'));
      ...
      Set<String> propBrandSpecialPricesToDelete = hivePropBrandSpecialPriceKeys
          .difference(firestorePropBrandSpecialPriceKeys);
      ...
      print(
          'Error synchronizing CustomerPropBrandSpecialPrice from Firebase to Hive: $e');
    }
  }
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> _synchronizeCustomerPropGroupSpecialPrice(
    List<QueryDocumentSnapshot<Map<String, dynamic>>>
        firestorePropGroupSpecialPrice,
    ...
    try {
      ...
      for (var doc in firestorePropGroupSpecialPrice) {
        ...
        // If the prop group special price exists in Hive, update it if needed
          ...
          await propGroupSpecialPrice.put(
              '$cmpCode$custGroupCode$propCode', updatedPropGroupSpecialPrice);
        }
      }
      Set<String> firestorePropGroupSpecialPriceKeys = Set.from(
          firestorePropGroupSpecialPrice.map((doc) =>
              '${doc['cmpCode']}${doc['custGroupCode']}${doc['propCode']}'));
      ...
      Set<String> propGroupSpecialPricesToDelete = hivePropGroupSpecialPriceKeys
          .difference(firestorePropGroupSpecialPriceKeys);
      ...
      print(
          'Error synchronizing CustomerPropGroupSpecialPrice from Firebase to Hive: $e');
    }
  }
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> _synchronizeCustomerPropCategSpecialPrice(
    List<QueryDocumentSnapshot<Map<String, dynamic>>>
        firestorePropCategSpecialPrice,
    ...
    try {
      ...
      for (var doc in firestorePropCategSpecialPrice) {
        ...
        // If the prop categ special price exists in Hive, update it if needed
          ...
          await propCategSpecialPrice.put(
              '$cmpCode$custPropCode$categCode', updatedPropCategSpecialPrice);
        }
      }
      Set<String> firestorePropCategSpecialPriceKeys = Set.from(
          firestorePropCategSpecialPrice.map((doc) =>
              '${doc['cmpCode']}${doc['custPropCode']}${doc['categCode']}'));
      ...
      Set<String> propCategSpecialPricesToDelete = hivePropCategSpecialPriceKeys
          .difference(firestorePropCategSpecialPriceKeys);
      ...
      print(
          'Error synchronizing CustomerPropCategSpecialPrice from Firebase to Hive: $e');
    }
  }
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<String> synchronizeDataPriceListsAutho() async {
    try {
      // Fetch data from Firestore
      var firestoreItems =
      ...
      // Synchronize data
      String err = await _synchronizePriceListAutho(
          firestoreItems.docs, pricelistsauthoBox);
      // Synchronize other data if needed
      if (err != '') {
        return err;
      }
  ...
  Future<String> _synchronizePriceListAutho(
    List<QueryDocumentSnapshot<Map<String, dynamic>>> firestorePriceLists,
    ...
    try {
      ...
      // Check for items in Hive that don't exist in Firestore and delete them
      Set<String> firestorePriceCodes = Set.from(firestorePriceLists.map(
          (doc) => '${doc['userCode']}${doc['cmpCode']}${doc['authoGroup']}'));
      Set<String> hivePriceCodes = Set.from(pricelistsBox.keys);
...
// Delete items in Hive that don't exist in Firestore
      ...
      print(
          'Error synchronizing PricesList Authorization from Firebase to Hive: $e');
      return '$e';
    }
  }
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> synchronizeDataCompaniesConnection() async {
    try {
      // Fetch data from Firestore
      var firestoreItems =
          await _firestore.collection('CompaniesConnection').get();

      // Open Hive boxes
      var copmpaniesconnBox =
          await Hive.openBox<CompaniesConnection>('companiesConnectionBox');
      // Open other boxes if needed

      // Synchronize data
      await _synchronizeCompaniesConnection(
          firestoreItems.docs, copmpaniesconnBox);
      // Synchronize other data if needed
  ...
  Future<void> _synchronizeCompaniesConnection(
    ...
    try {
      ...
      // Check for items in Hive that don't exist in Firestore and delete them
      Set<String> firestoreCompCodes =
          Set.from(firestoreCompaniesConn.map((doc) => doc['connectionID']));
...
//-------------------------------------------------------------------------------------------------
  ...
  Future<void> synchronizeDataCompaniesUsers() async {
    try {
      // Fetch data from Firestore
      var firestoreItems = await _firestore.collection('CompaniesUsers').get();

      // Open Hive boxes
      var copmpaniesusersBox =
          await Hive.openBox<CompaniesUsers>('companiesUsersBox');
      // Open other boxes if needed

      // Synchronize data
      await _synchronizeCompaniesUsers(firestoreItems.docs, copmpaniesusersBox);
      // Synchronize other data if needed
  ...
  Future<void> _synchronizeCompaniesUsers(
    List<QueryDocumentSnapshot<Map<String, dynamic>>> firestoreCompaniesUsers,
    Box<CompaniesUsers> compsBox,
  ) async {
    try {
      // Iterate over Firestore documents
      for (var doc in firestoreCompaniesUsers) {
        var userCode = doc['userCode'];
        var cmpCode = doc['cmpCode'];

        // Check if the item exists in Hive
        var hiveComp = compsBox.get('$userCode$cmpCode');
      ...
      // Check for items in Hive that don't exist in Firestore and delete them
      Set<String> firestoreCompCodes = Set.from(firestoreCompaniesUsers
          .map((doc) => '${doc['userCode']}${doc['cmpCode']}'));
      Set<String> hivePriceCodes = Set.from(compsBox.keys);
...
// Delete items in Hive that don't exist in Firestore
      ...
      print('Error synchronizing Companies Users from Firebase to Hive: $e');
    }
  }
}
