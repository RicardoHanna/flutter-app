...
class AdminAuthorizationsPage extends StatefulWidget {
  ...
  @override
  _AdminAuthorizationsPageState createState() =>
      _AdminAuthorizationsPageState();
}
...
class _AdminAuthorizationsPageState extends State<AdminAuthorizationsPage> {
  ...
  Stream<List<TranslationsClass>> get _usersStream =>
  ...
  void initState() {
    ...
    _userStreamController =
        StreamController<List<TranslationsClass>>.broadcast();
    _initUserStream();
  }
        ...
        try {
          ...
          if (user != null && user.groupcode != 1) {
            offlineUsers.add(TranslationsClass(
              ...
              translations: {
                'en': user.translations['en'] ?? 'empty',
                'ar': user.translations['ar'] ?? 'empty'
              },
            ));
          }
        } catch (e) {
...
// ... (your imports and other code)
  ...
  Future<void> _assignUserGroup(BuildContext context, String updatedGroupEn,
      String updatedGroupAr, int groupcode) async {
    var menuBox = await Hive.openBox<Menu>('menuBox');
    var adminSubMenuBox = await Hive.openBox<AdminSubMenu>('adminSubMenuBox');
    var synchronizeSubMenuBox =
...
//adminSubMenuBox.clear();
    ...
    var existingAuthorizations =
        authorizationBox.values.where((auth) => auth.groupcode == groupcode);
    ...
    var administratorMenu = allMenuItems.firstWhere((menu) =>
        menu.menuname == 'Administrator' || menu.menuarname == 'الإدارة');
    //var synchronizeMenu = allMenuItems.firstWhere((menu) => menu.menuname == 'Synchronize' || menu.menuarname == 'تزامن');

    var langdetec = AppLocalizations.of(context)!.language == "English"
        ? administratorMenu.menuname
        : administratorMenu.menuarname;
      ...
      builder: (BuildContext context) {
        return StatefulBuilder(
          builder: (BuildContext context, StateSetter setState) {
            return AlertDialog(
              title: Text(AppLocalizations.of(context)!.assignusergrouptomenu,
                  style: _appTextStyle),
              content: SingleChildScrollView(
                ...
                child: Column(
                  children: [
                      ...
                      onChanged: (value) {
                        setState(() {
                          if (value!) {
                            ...
                            int compositeKey = _generateCompositeKey(
                                administratorMenu.menucode, groupcode);
                            authorizationBox.put(
                                compositeKey,
                                Authorization(
                                    menucode: administratorMenu.menucode,
                                    groupcode: groupcode));
                          ...
                          for (var submenu in adminSubMenuBox.values) {
                            if (value) {
                              ...
                              int compositeKey = _generateCompositeKey(
                                  submenu.groupcode, groupcode);
                              authorizationBox.put(
                                  compositeKey,
                                  Authorization(
                                      menucode: submenu.groupcode,
                                      groupcode: groupcode));
                            } else {
                              // Unmark the submenu
                    ...
                    ExpansionTile(
                      title: Text(
                        AppLocalizations.of(context)!.adminmenus,
                        style: _appTextStyle,
                      ),
                      children: [
                        for (var submenu in adminSubMenuBox.values)
      ...
      for (var submenu in synchronizeSubMenuBox.values) {
        if (value) {
          ...
          authorizationBox.delete(compositeKey);
                    ...
                    for (var menuItem in allMenuItems.where((menu) =>
                        menu.menuname != 'Administrator' ||
                        menu.menuarname != 'الإدارة'))
                      CheckboxListTile(
                        title: Text(
                ...
                TextButton(
                  onPressed: () async {
                    ...
                    for (var selectedItem in selectedItems) {
                      int compositeKey =
                          _generateCompositeKey(selectedItem, groupcode);
                      authorizationBox.put(
                          compositeKey,
                          Authorization(
                              menucode: selectedItem, groupcode: groupcode));
                    }
                    print('ricop');
