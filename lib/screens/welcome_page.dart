...
//itemsBox.clear();
  ...
  Future<void> _loadDefaultCompanyCode() async {
    try {
      ...
      var companiesUser = companiesUsersBox.values
          .firstWhere((element) => element.userCode == usercode);
      if (companiesUser != null) {
        setState(() {
          ...
          selectedCompany = companies
              .firstWhere((company) => company.cmpCode == selectedCompanyCode)
              .cmpName;
        });
      }
    } catch (error) {
  ...
  void saveProfile(String localPath) async {
    ...
    UploadTask uploadTask = storageReference.putFile(newImg);
    await _uploadImageToFireStore('Updating');
    ...
    await _uploadImageToFireStore(downloadURL);
    // Save download URL in Firestore
  }

  Future<void> _uploadImageToFireStore(String downloadURL) async {
                  ...
                  StreamBuilder(
                    ...
                    builder: (BuildContext context,
                      ...
                      if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
                        // Return a widget for when no data is available
                      ...
                      // For example, to access the first document's data
                      Map<String, dynamic> userData = documents[0].data()!;
                      // Do something with the user data

                      // Return your UI based on the data
                      if (userData['imageLink'] == 'Updating') {
                        return Center(
          ...
          content: DropdownButtonFormField<String>(
            ...
            onChanged: (value) {
              setState(() {
                ...
                selectedCompanyCode = companies
                    .firstWhere((company) => company.cmpName == value)
                    .cmpCode;
  ...
  Future<void> _updateDefaultCompanyCode(String companyCode) async {
    ...
    try {
      ...
      var companiesUser = companiesUsersBox.values
          .where((element) => element.userCode == usercode);
      for (var companyUser in companiesUser) {
        ...
        companiesUsersBox.put(
            '$userCode$companyNameEachRecordToUpdate', companyUser);
      }
    } catch (error) {
