...
if (rowsItemUOM && rowsItemUOM.length) {
        ...
        if(customersTable === 'Customers'){
          ...
          if (rowsCustomerItemsSpecialPrice && rowsCustomerItemsSpecialPrice.length) {
            ...
            for (let i = 0; i < rowsCustomerItemsSpecialPrice.length; i++) {
              ...
              const compositeIdentifierValue = identifierFieldsCustomerItemsSpecialPrice.map(field => updatedRecord[field]).join('-');
            ...
            const documentsToDeleteCustomerItemsSpecialPrice = firestoreDocumentsCustomerItemsSpecialPrice.filter(doc =>
              !identifierValuesCustomerItemsSpecialPrice.includes(identifierFieldsCustomerItemsSpecialPrice.map(field => doc[field]).join('-'))
            );
          
            // Delete Firestore documents that are not in SQL Server results
            for (const docToDelete of documentsToDeleteCustomerItemsSpecialPrice) {
              const compositeIdentifierValueToDelete = identifierFieldsCustomerItemsSpecialPrice.map(field => docToDelete[field]).join('-');
          ...
          if (rowsCustomerBrandSpecialPrice && rowsCustomerBrandSpecialPrice.length) {
            ...
            for (let i = 0; i < rowsCustomerBrandSpecialPrice.length; i++) {
              ...
              const compositeIdentifierValue = identifierFieldsCustomerBrandSpecialPrice.map(field => updatedRecord[field]).join('-');
            ...
            const documentsToDeleteCustomerBrandSpecialPrice = firestoreDocumentsCustomerBrandSpecialPrice.filter(doc =>
              !identifierValuesCustomerBrandSpecialPrice.includes(identifierFieldsCustomerBrandSpecialPrice.map(field => doc[field]).join('-'))
            );
          
            // Delete Firestore documents that are not in SQL Server results
            for (const docToDelete of documentsToDeleteCustomerBrandSpecialPrice) {
              const compositeIdentifierValueToDelete = identifierFieldsCustomerBrandSpecialPrice.map(field => docToDelete[field]).join('-');
          ...
          if (rowsCustomerGroupSpecialPrice && rowsCustomerGroupSpecialPrice.length) {
            ...
            for (let i = 0; i < rowsCustomerGroupSpecialPrice.length; i++) {
              ...
              const compositeIdentifierValue = identifierFieldsCustomerGroupSpecialPrice.map(field => updatedRecord[field]).join('-');
            ...
            const documentsToDeleteCustomerGroupSpecialPrice = firestoreDocumentsCustomerGroupSpecialPrice.filter(doc =>
              !identifierValuesCustomerGroupSpecialPrice.includes(identifierFieldsCustomerGroupSpecialPrice.map(field => doc[field]).join('-'))
            );
          
            // Delete Firestore documents that are not in SQL Server results
            for (const docToDelete of documentsToDeleteCustomerGroupSpecialPrice) {
              const compositeIdentifierValueToDelete = identifierFieldsCustomerGroupSpecialPrice.map(field => docToDelete[field]).join('-');
          ...
          if (rowsCustomerCategSpecialPrice && rowsCustomerCategSpecialPrice.length) {
            ...
            for (let i = 0; i < rowsCustomerCategSpecialPrice.length; i++) {
              ...
              const compositeIdentifierValue = identifierFieldsCustomerCategSpecialPrice.map(field => updatedRecord[field]).join('-');
            ...
            const documentsToDeleteCustomerCategSpecialPrice = firestoreDocumentsCustomerCategSpecialPrice.filter(doc =>
              !identifierValuesCustomerCategSpecialPrice.includes(identifierFieldsCustomerCategSpecialPrice.map(field => doc[field]).join('-'))
            );
          
            // Delete Firestore documents that are not in SQL Server results
            for (const docToDelete of documentsToDeleteCustomerCategSpecialPrice) {
              const compositeIdentifierValueToDelete = identifierFieldsCustomerCategSpecialPrice.map(field => docToDelete[field]).join('-');
          ...
          if (rowsCustomerGroupItemsSpecialPrice && rowsCustomerGroupItemsSpecialPrice.length) {
            ...
            for (let i = 0; i < rowsCustomerGroupItemsSpecialPrice.length; i++) {
              ...
              const compositeIdentifierValue = identifierFieldsCustomerGroupItemsSpecialPrice.map(field => updatedRecord[field]).join('-');
            ...
            const documentsToDeleteCustomerGroupItemsSpecialPrice = firestoreDocumentsCustomerGroupItemsSpecialPrice.filter(doc =>
              !identifierValuesCustomerGroupItemsSpecialPrice.includes(identifierFieldsCustomerGroupItemsSpecialPrice.map(field => doc[field]).join('-'))
            );
          
            // Delete Firestore documents that are not in SQL Server results
            for (const docToDelete of documentsToDeleteCustomerGroupItemsSpecialPrice) {
              const compositeIdentifierValueToDelete = identifierFieldsCustomerGroupItemsSpecialPrice.map(field => docToDelete[field]).join('-');
          ...
          if (rowsCustomerGroupBrandSpecialPrice && rowsCustomerGroupBrandSpecialPrice.length) {
            ...
            for (let i = 0; i < rowsCustomerGroupBrandSpecialPrice.length; i++) {
              ...
              const compositeIdentifierValue = identifierFieldsCustomerGroupBrandSpecialPrice.map(field => updatedRecord[field]).join('-');
            ...
            const documentsToDeleteCustomerGroupBrandSpecialPrice = firestoreDocumentsCustomerGroupBrandSpecialPrice.filter(doc =>
              !identifierValuesCustomerGroupBrandSpecialPrice.includes(identifierFieldsCustomerGroupBrandSpecialPrice.map(field => doc[field]).join('-'))
            );
          
            // Delete Firestore documents that are not in SQL Server results
            for (const docToDelete of documentsToDeleteCustomerGroupBrandSpecialPrice) {
              const compositeIdentifierValueToDelete = identifierFieldsCustomerGroupBrandSpecialPrice.map(field => docToDelete[field]).join('-');
          ...
          if (rowsCustomerGroupGroupSpecialPrice && rowsCustomerGroupGroupSpecialPrice.length) {
            ...
            for (let i = 0; i < rowsCustomerGroupGroupSpecialPrice.length; i++) {
              ...
              const compositeIdentifierValue = identifierFieldsCustomerGroupGroupSpecialPrice.map(field => updatedRecord[field]).join('-');
            ...
            const documentsToDeleteCustomerGroupGroupSpecialPrice = firestoreDocumentsCustomerGroupGroupSpecialPrice.filter(doc =>
              !identifierValuesCustomerGroupGroupSpecialPrice.includes(identifierFieldsCustomerGroupGroupSpecialPrice.map(field => doc[field]).join('-'))
            );
          
            // Delete Firestore documents that are not in SQL Server results
            for (const docToDelete of documentsToDeleteCustomerGroupGroupSpecialPrice) {
              const compositeIdentifierValueToDelete = identifierFieldsCustomerGroupGroupSpecialPrice.map(field => docToDelete[field]).join('-');
          ...
          if (rowsCustomerGroupCategSpecialPrice && rowsCustomerGroupCategSpecialPrice.length) {
            ...
            for (let i = 0; i < rowsCustomerGroupCategSpecialPrice.length; i++) {
              ...
              const compositeIdentifierValue = identifierFieldsCustomerGroupCategSpecialPrice.map(field => updatedRecord[field]).join('-');
            ...
            const documentsToDeleteCustomerGroupCategSpecialPrice = firestoreDocumentsCustomerGroupCategSpecialPrice.filter(doc =>
              !identifierValuesCustomerGroupCategSpecialPrice.includes(identifierFieldsCustomerGroupCategSpecialPrice.map(field => doc[field]).join('-'))
            );
          
            // Delete Firestore documents that are not in SQL Server results
            for (const docToDelete of documentsToDeleteCustomerGroupCategSpecialPrice) {
              const compositeIdentifierValueToDelete = identifierFieldsCustomerGroupCategSpecialPrice.map(field => docToDelete[field]).join('-');
          ...
          if (rowsCustomerPropItemsSpecialPrice && rowsCustomerPropItemsSpecialPrice.length) {
            ...
            for (let i = 0; i < rowsCustomerPropItemsSpecialPrice.length; i++) {
              ...
              const compositeIdentifierValue = identifierFieldsCustomerPropItemsSpecialPrice.map(field => updatedRecord[field]).join('-');
            ...
            const documentsToDeleteCustomerPropItemsSpecialPrice = firestoreDocumentsCustomerPropItemsSpecialPrice.filter(doc =>
              !identifierValuesCustomerPropItemsSpecialPrice.includes(identifierFieldsCustomerPropItemsSpecialPrice.map(field => doc[field]).join('-'))
            );
          
            // Delete Firestore documents that are not in SQL Server results
            for (const docToDelete of documentsToDeleteCustomerPropItemsSpecialPrice) {
              const compositeIdentifierValueToDelete = identifierFieldsCustomerPropItemsSpecialPrice.map(field => docToDelete[field]).join('-');
          ...
          if (rowsCustomerPropBrandSpecialPrice && rowsCustomerPropBrandSpecialPrice.length) {
            ...
            for (let i = 0; i < rowsCustomerPropBrandSpecialPrice.length; i++) {
              ...
              const compositeIdentifierValue = identifierFieldsCustomerPropBrandSpecialPrice.map(field => updatedRecord[field]).join('-');
            ...
            const documentsToDeleteCustomerPropBrandSpecialPrice = firestoreDocumentsCustomerPropBrandSpecialPrice.filter(doc =>
              !identifierValuesCustomerPropBrandSpecialPrice.includes(identifierFieldsCustomerPropBrandSpecialPrice.map(field => doc[field]).join('-'))
            );
          
            // Delete Firestore documents that are not in SQL Server results
            for (const docToDelete of documentsToDeleteCustomerPropBrandSpecialPrice) {
              const compositeIdentifierValueToDelete = identifierFieldsCustomerPropBrandSpecialPrice.map(field => docToDelete[field]).join('-');
          ...
          if (rowsCustomerPropGroupSpecialPrice && rowsCustomerPropGroupSpecialPrice.length) {
            ...
            for (let i = 0; i < rowsCustomerPropGroupSpecialPrice.length; i++) {
              ...
              const compositeIdentifierValue = identifierFieldsCustomerPropGroupSpecialPrice.map(field => updatedRecord[field]).join('-');
            ...
            const documentsToDeleteCustomerPropGroupSpecialPrice = firestoreDocumentsCustomerPropGroupSpecialPrice.filter(doc =>
              !identifierValuesCustomerPropGroupSpecialPrice.includes(identifierFieldsCustomerPropGroupSpecialPrice.map(field => doc[field]).join('-'))
            );
          
            // Delete Firestore documents that are not in SQL Server results
            for (const docToDelete of documentsToDeleteCustomerPropGroupSpecialPrice) {
              const compositeIdentifierValueToDelete = identifierFieldsCustomerPropGroupSpecialPrice.map(field => docToDelete[field]).join('-');
          ...
          if (rowsCustomerPropCategSpecialPrice && rowsCustomerPropCategSpecialPrice.length) {
            ...
            for (let i = 0; i < rowsCustomerPropCategSpecialPrice.length; i++) {
              ...
              const compositeIdentifierValue = identifierFieldsCustomerPropCategSpecialPrice.map(field => updatedRecord[field]).join('-');
            ...
            const documentsToDeleteCustomerPropCategSpecialPrice = firestoreDocumentsCustomerPropCategSpecialPrice.filter(doc =>
              !identifierValuesCustomerPropCategSpecialPrice.includes(identifierFieldsCustomerPropCategSpecialPrice.map(field => doc[field]).join('-'))
            );
          
            // Delete Firestore documents that are not in SQL Server results
            for (const docToDelete of documentsToDeleteCustomerPropCategSpecialPrice) {
              const compositeIdentifierValueToDelete = identifierFieldsCustomerPropCategSpecialPrice.map(field => docToDelete[field]).join('-');
  ...
  for (let i = 0; i < rowsSalesEmployeesCustomers.length; i++) {
    ...
    const identifierValue = Object.values(updatedRecord).filter(value => identifierFieldsSalesEmployeesCustomers.includes(value))[0];
  ...
  const documentsToDeleteSalesEmployeesCustomers = firestoreDocumentsSalesEmployeesCustomers.filter(doc =>
    !identifierValuesSalesEmployeesCustomers.includes(Object.values(doc).filter(value => identifierFieldsSalesEmployeesCustomers.includes(value))[0])
  );

  // Delete Firestore documents that are not in SQL Server results
  for (const docToDelete of documentsToDeleteSalesEmployeesCustomers) {
    const identifierValue = Object.values(docToDelete).filter(value => identifierFieldsSalesEmployeesCustomers.includes(value))[0];
...
if (rowsSalesEmployeesDepartments && rowsSalesEmployeesDepartments.length) {
  ...
  for (let i = 0; i < rowsSalesEmployeesDepartments.length; i++) {
    ...
    const identifierValue = Object.values(updatedRecord).filter(value => identifierFieldsSalesEmployeesDepartments.includes(value))[0];
  ...
  const documentsToDeleteSalesEmployeesDepartments = firestoreDocumentsSalesEmployeesDepartments.filter(doc =>
    !identifierValuesSalesEmployeesDepartments.includes(Object.values(doc).filter(value => identifierFieldsSalesEmployeesDepartments.includes(value))[0])
  );

  // Delete Firestore documents that are not in SQL Server results
  for (const docToDelete of documentsToDeleteSalesEmployeesDepartments) {
    const identifierValue = Object.values(docToDelete).filter(value => identifierFieldsSalesEmployeesDepartments.includes(value))[0];
...
if (rowsSalesEmployeesItemsBrands && rowsSalesEmployeesItemsBrands.length) {
  ...
  for (let i = 0; i < rowsSalesEmployeesItemsBrands.length; i++) {
    ...
    const identifierValue = Object.values(updatedRecord).filter(value => identifierFieldsSalesEmployeesItemsBrands.includes(value))[0];
  ...
  const documentsToDeleteSalesEmployeesItemsBrands = firestoreDocumentsSalesEmployeesItemsBrands.filter(doc =>
    !identifierValuesSalesEmployeesItemsBrands.includes(Object.values(doc).filter(value => identifierFieldsSalesEmployeesItemsBrands.includes(value))[0])
  );

  // Delete Firestore documents that are not in SQL Server results
  for (const docToDelete of documentsToDeleteSalesEmployeesItemsBrands) {
    const identifierValue = Object.values(docToDelete).filter(value => identifierFieldsSalesEmployeesItemsBrands.includes(value))[0];
...
if (rowsSalesEmployeesItemsCategories && rowsSalesEmployeesItemsCategories.length) {
  ...
  for (let i = 0; I < rowsSalesEmployeesItemsCategories.length; i++) {
    ...
    const identifierValue = Object.values(updatedRecord).filter(value => identifierFieldsSalesEmployeesItemsCategories.includes(value))[0];
  ...
  const documentsToDeleteSalesEmployeesItemsCategories = firestoreDocumentsSalesEmployeesItemsCategories.filter(doc =>
    !identifierValuesSalesEmployeesItemsCategories.includes(Object.values(doc).filter(value => identifierFieldsSalesEmployeesItemsCategories.includes(value))[0])
  );

  // Delete Firestore documents that are not in SQL Server results
  for (const docToDelete of documentsToDeleteSalesEmployeesItemsCategories) {
    const identifierValue = Object.values(docToDelete).filter(value => identifierFieldsSalesEmployeesItemsCategories.includes(value))[0];
...
if (rowsSalesEmployeesItemsGroups && rowsSalesEmployeesItemsGroups.length) {
  ...
  for (let i = 0; I < rowsSalesEmployeesItemsGroups.length; i++) {
    ...
    const identifierValue = Object.values(updatedRecord).filter(value => identifierFieldsSalesEmployeesItemsGroups.includes(value))[0];
  ...
  const documentsToDeleteSalesEmployeesItemsGroups = firestoreDocumentsSalesEmployeesItemsGroups.filter(doc =>
    !identifierValuesSalesEmployeesItemsGroups.includes(Object.values(doc).filter(value => identifierFieldsSalesEmployeesItemsGroups.includes(value))[0])
  );

  // Delete Firestore documents that are not in SQL Server results
  for (const docToDelete of documentsToDeleteSalesEmployeesItemsGroups) {
    const identifierValue = Object.values(docToDelete).filter(value => identifierFieldsSalesEmployeesItemsGroups.includes(value))[0];
...
if (rowsSalesEmployeesItems && rowsSalesEmployeesItems.length) {
  ...
  for (let i = 0; I < rowsSalesEmployeesItems.length; i++) {
    ...
    const identifierValue = Object.values(updatedRecord).filter(value => identifierFieldsSalesEmployeesItems.includes(value))[0];
  ...
  const documentsToDeleteSalesEmployeesItems = firestoreDocumentsSalesEmployeesItems.filter(doc =>
    !identifierValuesSalesEmployeesItems.includes(Object.values(doc).filter(value => identifierFieldsSalesEmployeesItems.includes(value))[0])
  );

  // Delete Firestore documents that are not in SQL Server results
  for (const docToDelete of documentsToDeleteSalesEmployeesItems) {
    const identifierValue = Object.values(docToDelete).filter(value => identifierFieldsSalesEmployeesItems.includes(value))[0];
...
if (rowsUsersSalesEmployees && rowsUsersSalesEmployees.length) {
  ...
  for (let i = 0; i < rowsUsersSalesEmployees.length; i++) {
    ...
    const identifierValue = Object.values(updatedRecord).filter(value => identifierFieldsUsersSalesEmployees.includes(value))[0];
  ...
  const documentsToDeleteUsersSalesEmployees = firestoreDocumentsUsersSalesEmployees.filter(doc =>
    !identifierValuesUsersSalesEmployees.includes(Object.values(doc).filter(value => identifierFieldsUsersSalesEmployees.includes(value))[0])
  );

  // Delete Firestore documents that are not in SQL Server results
  for (const docToDelete of documentsToDeleteUsersSalesEmployees) {
    const identifierValue = Object.values(docToDelete).filter(value => identifierFieldsUsersSalesEmployees.includes(value))[0];
